...
c_default_subcommand="${c_default_subcommand:-_default}"

c_sub_cmd="${next:-$c_default_subcommand}" # if no argument get the default (set in g_locations.sh)

$DEBUG && echo "s_sub_cmd: $s_sub_cmd($s_dest_cmd $s_dest_subcmd_name) args(${#args[@]}): ${args[@]:+${args[@]}}"
